...
#
# Step-1. Cut grid
gmt grdcut earth_relief_05m.grd -R263/278/7/17 -Gmat_relief.nc
# Step-2. Create color palette
gmt grd2cpt @mat_relief.nc -Crainbow > rainbowMAT.cpt
# Step-3. generate a file
ps=MAT3D165.ps
# Step-4. Add contour
gmt grdcontour geoid.egm96.grd -JM3.5i -R263/278/7/17 -p165/30 -C1 -A5 -Gd3c -Wthinnest,dimgray -Y3c -U/-0.5c/-1c/"Data: 2 min World Geoid Image 9.2, ETOPO 5 arc min grid" -P -K > $ps
# Step-5. Add coastlines, ticks, rose,
pscoast -J -R -p165/30 -B4/2NESW -Gdimgray -O -K -T168/42/1c \
    --MAP_ANNOT_OFFSET=0.1c >> $ps
# Step-6. Add 3D
grdview mat_relief.nc -J -R -JZ2i -CrainbowMAT.cpt -p165/30 -Qsm -N-7500+glightgray \
...
0.0 9.0 Region: Guatemala Trench
EOF
...
gmt psconvert MAT3D165.ps -A0.4c -E720 -Tj -P -Z
# Step-10. Clean up
rm -f rainbowMAT.cpt
