...
#
# Step-1. Cut grid
gmt grdcut earth_relief_05m.grd -R141.5/152/4/17 -Gmt3d_relief.nc
# Step-2. Create color palette
gmt grd2cpt @mt3d_relief.nc -Crainbow > rainbowMT.cpt
# Step-3. generate a file
ps=MT3D165.ps
# Step-4. Add contour
gmt grdcontour geoid.egm96.grd -JM3.5i \
    -R141.5/152/4/17 \
    -p250/45 -C1 -A5 -Gd3c -Wthinnest,dimgray -Y3c \
    -U/-0.8c/-1c \
    -P -K > $ps
# Step-5. Add coastlines, ticks, rose,
pscoast -J -R -p250/45 -B4/2NESW -Gdimgray -O -K -T148/32/1c \
    --MAP_ANNOT_OFFSET=0.1c >> $ps
# Step-6. Add 3D
grdview mt3d_relief.nc -J -R -JZ3c -CrainbowMT.cpt \
    -p250/45 -Qsm -N-11000+glightgray \
...
# Step-7. Add GMT logo
gmt logo -Dx6.0/0.0+o0.0c/-6.0c+w2c -O -K >> $ps
# Step-8. Add title
gmt pstext -R0/10/0/10 -Jx1 -X-0.8c -Y1.5c -N -O -K \
    -F+f9p,Palatino-Roman,black+jLB >> $ps << EOF
0.0 11.0 Composite overlay: 3D topographic mesh model on top of 2D geoid contour plot
0.0 10.5 Region: Mariana Trench
0.0 10.0 Data: ETOPO 5 arc min grid, Geoid Image 9.2 5 min grid
EOF
gmt pstext -R0/10/0/10 -Jx1 -X0.0c -Y0.0c -N -O \
    -F+f10p,Palatino-Roman,dimgray+jLB >> $ps << EOF
7.0 9.0 Perspective view azimuth: 250/45\232
EOF
# Step-9. Convert to image file using GhostScript (portrait orientation, 720 dpi)
gmt psconvert MT3D165.ps -A0.4c -E720 -Tj -P -Z
# Step-10. Clean up
rm -f mt3d_relief.nc rainbowMT.cpt
