...
#
# Step-1. Cut grid
gmt grdcut earth_relief_05m.grd -R128/150/30/46 -Gjt3d_relief.nc
# Step-2. Create color palette
gmt grd2cpt @jt3d_relief.nc -Crainbow > rainbowJT.cpt
# Step-3. generate a file
ps=JT3D165.ps
# Step-4. Add contour
gmt grdcontour geoid.egm96.grd -JM3.5i \
    -R128/150/30/46 \
    -p165/45 -C1 -A5 -Gd3c -Wthinnest,dimgray -Y3c \
    -U/-0.8c/-1c \
    -P -K > $ps
# Step-5. Add coastlines, ticks, rose,
pscoast -J -R -p165/45 -B4/2NESW -Gdimgray -O -K -T148/32/1c \
    --MAP_ANNOT_OFFSET=0.1c >> $ps
# Step-6. Add 3D
grdview jt3d_relief.nc -J -R -JZ4c -CrainbowJT.cpt \
    -p165/45 -Qsm -N-9000+glightgray \
...
0.0 9.0 Region: Japan Trench
0.0 8.5 Data: ETOPO 5 arc min grid, World Geoid Image 9.2 5 min grid
EOF
...
gmt psconvert JT3D165.ps -A0.4c -E720 -Tj -P -Z
# Step-10. Clean up
